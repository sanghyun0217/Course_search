...
class Sidebar extends React.Component {
  ...
  getKeywords() {
    ...
    for(const course of this.props.courses) {
      keywords = keywords.filter( function(v) {
        return !course.keywords.includes(v)
      })
      course.keywords.forEach(function (v) {
        keywords.push(v)
      })
    }

    keywordsOption.push(<option key="All">All</option>)

    for(const keyword of keywords) {
      keywordsOption.push(<option key={keyword}>{keyword}</option>)
    }
